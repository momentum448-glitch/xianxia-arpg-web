...
const FIELD_EDGE_TEX = 'qc-field-edge-kit-a';
const FIELD_EDGE_PATH = 'assets/c4/environment/settlement/env_field_edge_kit_a.webp';
...
export class VillageTopologyQcFieldEdgeScene extends VillageTopologyQcHealerActivityScene {
  ...
  private replaceSouthernFieldBlockout(): void {
    ...
    // Minimal proof only: replace the two right-side agriculture blockout rows.
    // Do not touch the left residential field rows, route, houses or gameplay space.
  ...
  private removeEdgeConnectedDarkMatte(textureKey: string): void {
    ...
    const isDark = (index: number): boolean => {
      ...
      return pixels[offset + 3] > 8 && pixels[offset] < 36 && pixels[offset + 1] < 36 && pixels[offset + 2] < 36;
    };
    const enqueue = (x: number, y: number): void => {
      if (x < 0 || x >= width || y < 0 || y >= height) return;
      const index = y * width + x;
      if (visited[index] || !isDark(index)) return;
      visited[index] = 1;
      queue[tail++] = index;
    };
  ...
  private refreshFieldQcCopy(): void {
    ...
    const replacements = new Map<string, string>([
      ['V2-A · HEALER ACTIVITY KIT A · GARDEN POS', 'V2-A · FIELD EDGE KIT A · PROOF'],
      ['water/bridge locked · herb garden moved to healer-left edge', 'healer locked · one southern field-edge cluster only'],
      ['QC: vườn bên trái nhà Dược Sư đã hợp lý? 0.65x pocket còn thoáng?', 'QC: 1.0x ruộng hòa scene? 0.65x mép làng tự nhiên, không hút mắt?'],
      ['NƯỚC + CẦU: PASS · HEALER ACTIVITY: POSITION REVISE', 'HEALER: PASS · FIELD EDGE: PROOF'],
    ]);
